<?php
    ...
    $dbName = "movie";
        ...
        elseif(empty($contents)):
            echo "<script>alert('개봉일을 입력해주세요.');history.back();</script>";
        elseif(empty($link)):
            echo "<script>alert('파일 링크을 입력해주세요.');history.back();</script>";
        else:
            $sql = "INSERT INTO g5_write_gallery(ca_name, wr_subject, wr_content, wr_link1, wr_name, wr_datetime)";
            $sql .= " VALUES('관리자', '{$title}', '{$contents}', '{$link}', '{$file_name}','{$regTime}');";
            
            $result = $dbConnect->query($sql);
            print_r($result);
            if($result){
                echo "<script>alert('등록이 완료되었습니다.');location.href='./coming_soon.php';</script>";
            }else{
                echo "fail";
            }
        endif;
